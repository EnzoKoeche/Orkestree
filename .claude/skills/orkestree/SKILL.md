...
**Mecanismo A — Prisma select role-aware** (em cada service):
- `INTERNAL_SELECT` para PRIVILEGED (OWNER/ADMIN): inclui `totalCost`, `internalCost`, `notes` interno, margem.
- `CLIENT_SELECT` para FINANCEIRO/OPERACIONAL/CLIENTE: exclui campos sensíveis.
- Helper `selectForRole(role)` escolhe o select.
...
- Adicionar campo sensível = uma entrada no registry, mais o select do service que retorna ele.
...
2. Selects são role-aware? Campo sensível novo está no registry?
