...
Dois selects no service:

- `PROPOSAL_INTERNAL_SELECT` — OWNER/ADMIN: inclui `totalCost`, `notes`, e `approvedBy/rejectedBy` memberships
- `PROPOSAL_CLIENT_SELECT` — FINANCEIRO/OPERACIONAL/CLIENTE: sem `totalCost`, sem `notes` interno

Item selects análogos (`PROPOSAL_ITEM_INTERNAL_SELECT` inclui `internalCost`).

`FieldFilterInterceptor` global (Mechanismo B) é defense-in-depth via `SENSITIVE_FIELD_REGISTRY`.
