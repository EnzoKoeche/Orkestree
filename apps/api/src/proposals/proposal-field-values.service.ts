...
const PROPOSAL_FIELD_VALUE_SELECT = {
    id: true,
    customFieldId: true,
    valueText: true,
    valueNumber: true,
...
} satisfies Prisma.ProposalFieldValueSelect;

// ─────────────────────────────────────────────────────────────────────────────
// ProposalFieldValuesService
//
// Handles validation and persistence of custom field values on Proposals.
// PROPOSAL fields are not scoped by serviceTypeId — any active CustomField
// with target = PROPOSAL and companyId match is applicable. Mirrors
// ClientFieldValuesService for consistency.
//
// Mutations are only valid while Proposal.status = DRAFT — that gate is
// enforced by the caller (ProposalsService) which holds a SELECT FOR UPDATE
// on the proposal row before invoking writeFieldValues.
// ─────────────────────────────────────────────────────────────────────────────

@Injectable()
export class ProposalFieldValuesService {
    constructor(private readonly prisma: PrismaService) { }

    // ── Public API ────────────────────────────────────────────────────────────

    /**
     * Loads applicable proposal fields and validates the submitted items.
    ...
    /**
     * Upserts field values for a proposal. Call after validateAndLoad.
     * Must be called inside an open Prisma transaction.
     */
    async writeFieldValues(
        ...
        await Promise.all(
            items.map((item) => {
                ...
                return tx.proposalFieldValue.upsert({
                    where: {
                        proposalId_customFieldId: {
                            proposalId,
    ...
    /**
     * Reads all field values for a proposal, including field metadata.
     * Does NOT include companyId or proposalId in the returned shape.
     */
    async getFieldValues(companyId: string, proposalId: string) {
        return this.prisma.proposalFieldValue.findMany({
            where: { companyId, proposalId },
            select: PROPOSAL_FIELD_VALUE_SELECT,
    ...
    private validateFieldValues(
        ...
        // Reject field IDs not applicable to this company's proposal fields
        for (const item of items) {
            if (!fieldMap.has(item.customFieldId)) {
                throw new BadRequestException(
                    `Custom field "${item.customFieldId}" is not a valid proposal field for this company.`,
                );
            }
        }
        ...
        if (missingRequired.length > 0) {
            throw new UnprocessableEntityException(
                `Required proposal fields missing: ${missingRequired.map((f) => f.code).join(', ')}.`,
            );
        }
    ...
    private validateSingleFieldValue(
        ...
        item: SetProposalFieldValueItemDto,
    ): void {
        switch (field.type) {
